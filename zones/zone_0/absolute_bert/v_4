...
class Absolute_attention(nn.Module):
  def __init__(self, dim=512, num_heads=8, hidden_dim=None, time_dim=64):
    super().__init__()
...
class Absolute_bert(nn.Module):
  def __init__(self, vocab_size, dim=512, num_heads=8, hidden_dim=None, depth=8, log_granularity=[6,6,6,6,6,6,6,6], attention_type=Absolute_attention, dtype=torch.float):
    super().__init__()
  ...
  def forward(self, input_ids, attention_mask, **kwargs):
    ...
    for layer in self.layers:
      ...
      tensor = self.layer_norm(tensor + output)

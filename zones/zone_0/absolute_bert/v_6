...
class Absolute_attention(nn.Module):
  def __init__(self, dim=512, num_heads=8, hidden_dim=None, time_dim=64, linear_dim=3*512):
    super().__init__()
    ...
    self.k_bias = nn.Parameter(torch.zeros(num_heads * self.hidden_dim))
  ...
  def forward(self, tensor, attention_mask):
    ...
    k = (self.K(tensor) - self.k_bias.exp()).view(*batch_length, self.num_heads, self.hidden_dim)  # bthd
    ...
    lin_output = self.dropout(self.linear_out(self.linear_in(intermediate)))

    return self.layer_norm_lin(intermediate + lin_output)

class Absolute_bert(nn.Module):
  def __init__(self, vocab_size, dim=512, num_heads=8, hidden_dim=None, linear_dim=3*512, depth=8, log_granularity=[6,6,6,6,6,6,6,6], attention_type=Absolute_attention, dtype=torch.float):
    super().__init__()

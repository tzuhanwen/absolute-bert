...
class Multihead_Lima(nn.Module):
  def __init__(self, dim=64, hidden_dim=32, num_heads=4, fix_max_value=1):
    ...
    self.Theta = torch.nn.Parameter(torch.rand(self.num_heads, self.hidden_dim)*torch.pi*2 - torch.pi, requires_grad=True)
  ...
  def forward(self, theta):
    ...
    o = self.M + scale_controlling_coefficient * (-1 - b_a + h.cos() + b_a*(2*h).cos()) #shape: [batch_size, num_heads, , hidden_dim]
    raw_resulting_angle = o * self.Theta # 'bhm,hm->bhm'

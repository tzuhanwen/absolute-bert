...
class Multihead_GRUcell(nn.Module):
  def __init__(self, dim=64, hidden_dim=32, num_heads=4):
    ...
    self.sigmoid_w = nn.Parameter(torch.nn.init.xavier_normal_(torch.ones(self.num_heads, self.hidden_dim+self.dim, 2)))
    self.sigmoid_b = nn.Parameter(torch.nn.init.xavier_normal_(torch.ones(self.num_heads, 2)))
    self.sigmoid = nn.Sigmoid()
    self.tanh_w = nn.Parameter(torch.nn.init.xavier_normal_(torch.ones(self.num_heads, self.hidden_dim+self.dim, self.hidden_dim)))
    self.tanh_b = nn.Parameter(torch.nn.init.xavier_normal_(torch.ones(self.num_heads, self.hidden_dim)))
    self.tanh = nn.Tanh()
    
    self.out_w = nn.Parameter(torch.nn.init.xavier_normal_(torch.ones(self.num_heads, self.hidden_dim, self.dim)))
    self.out_b = nn.Parameter(torch.nn.init.xavier_normal_(torch.ones(self.num_heads, self.dim)))
...
class Output(nn.Module):
  def __init__(self,
               ...
               dim=64,
               rotary_denom=.5,
               local_coordinating=True,
               add_time_angles=False,
               embedding_initialize_method='xavier',
               embedding_initialize_scaling=5,
               **kwargs):
    super().__init__()
    self.dim = dim
    ...
    self.embedding_initialize_method = embedding_initialize_method
    self.embedding_initialize_scaling = embedding_initialize_scaling

    if embedding_initialize_method == 'xavier':
      self.embedding_real = nn.Embedding(vocab_size, self.dim, _weight=torch.nn.init.xavier_normal_(torch.ones([vocab_size, self.dim]))* self.embedding_initialize_scaling)
      self.embedding_imag = nn.Embedding(vocab_size, self.dim, _weight=torch.nn.init.xavier_normal_(torch.ones([vocab_size, self.dim]))* self.embedding_initialize_scaling)
    elif embedding_initialize_method == 'rand':
      self.embedding_real = torch.nn.Embedding(vocab_size, self.dim, _weight=torch.rand([vocab_size, self.dim])*2/np.sqrt(self.dim))
      self.embedding_imag = torch.nn.Embedding(vocab_size, self.dim, _weight=torch.rand([vocab_size, self.dim])*2/np.sqrt(self.dim))
    else:
      raise ValueError('embedding_initialize_method should be "xavier" or "rand"')
  ...
  def forward(self, angles, sources, word_angles=None):
    ...
    total_angles = self.tanh(total_angles)
...
class Rotator(nn.Module):
  def __init__(self, 
    ...
    self.predictor = Output(vocab_size,
                            dim=self.dim,
                            rotary_denom=rotary_denom, 
                            embedding_initialize_method=embedding_initialize_method,
                            **kwargs) # for rotate
    self.scale_fc = nn.Linear(self.history_dim*self.depth, self.scale_rank)
    self.scale_act = nn.GELU()
    self.angle_embedding = nn.Embedding(vocab_size, self.dim, _weight=torch.nn.init.xavier_normal_(torch.ones([vocab_size, self.dim])))

...
class Output(nn.Module):
  ...
  def forward(self, angles, sources, word_angles):
    time_angle = 1 / self.rotary_denom**(self.dimension_indices/self.dim) # shape: [1,dim]
    total_angles = time_angle + angles + word_angles
...
class Rotator(nn.Module):
  ...
  def forward(self, data, batch_sizes, **kwargs):
    ...
    histories = [history.expand(batch_sizes[1], -1, -1) for history in self.initial_history] # 近期記憶
    hippo_c = [self.initial_c.expand(batch_sizes[1], self.num_heads, -1) for _ in range(self.depth)] # 整體記憶
    ...
    for t, (batch_accum, batch_size, next_batch_size) in enumerate(zip(itertools.accumulate(batch_sizes, initial=0), batch_sizes, batch_sizes[1:])):
      ...
      for layer_num in range(self.depth):
        # print(f'{histories[layer_num].shape=}, {hippo_c[layer_num][:next_batch_size].shape=}')
        histories_hippos = torch.concat([histories[layer_num], hippo_c[layer_num]], dim=-1)[:next_batch_size] # shape: [batch_size of time t+1, num_heads, dim]
        # print(f'{(theta-accum).shape=}')
        output, history = self.limas[layer_num](theta-accum, histories_hippos)
        # print(f'{history.shape=}')
        
        hippo_c[layer_num] = self.hippos[layer_num](hippo_c[layer_num][:next_batch_size], history, t+1)[:next_batch_size]
        histories[layer_num] = history
        accum += output.sum(dim=1) # sum across heads, shape [batch_size of time t+1, dim]

      predicts = self.predictor(accum, batch, theta)

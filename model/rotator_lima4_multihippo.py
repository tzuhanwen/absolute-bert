...
class Multihead_Lima(nn.Module):
  def __init__(self, dim_in=64, dim_out=64, hidden_dim=32, num_heads=4, fix_max_value=1):
    super().__init__()
    self.dim_in = dim_in
    ...
    self.fc_in = torch.nn.Linear(self.num_heads*self.dim_in, self.num_heads*self.hidden_dim, bias=False)
    ...
    self.fc_out = torch.nn.Linear(self.num_heads*self.hidden_dim, self.num_heads*self.dim_out, bias=False)
  ...
  def forward(self, theta):
    ...
    transed_result_angle = self.fc_out(raw_resulting_angle.reshape(batch_size, -1)).view(batch_size, self.num_heads, self.dim_out)
...
class Rotator(nn.Module):
  def __init__(self, vocab_size, dim=64, hidden_dim=32, hippo_dim=16, num_hippo_heads=4, num_heads=4, depth=3, rotary_denom=.5, **kwargs):
    super().__init__()
    self.vocab_size = vocab_size
    self.dim = dim
    self.hidden_dim = hidden_dim
    ...
    self.limas = nn.ModuleList([Multihead_Lima(
      dim_in=self.dim+self.num_hippo_heads*self.hippo_dim,
      dim_out=self.dim,
      hidden_dim=self.hidden_dim,
      num_heads=self.num_heads
    ) for _ in range(self.depth)])
    self.hippo = Hippo(dim=self.dim, c_dim=self.hippo_dim, num_heads=self.num_hippo_heads)
  ...
  def forward(self, data, batch_sizes, **kwargs):
    ...
    hippo_c = [self.initial_c.expand(batch_sizes[0], self.num_heads, -1) for _ in range(self.depth)]
    # hippo_c = [torch.zeros([1, self.num_heads, self.hippo_dim]).expand(batch_sizes[0], -1, -1) for _ in range(self.depth)]
    ...
    for t, (batch_accum, batch_size, next_batch_size) in enumerate(zip(itertools.accumulate(batch_sizes, initial=0), batch_sizes, batch_sizes[1:])):
      ...
      theta = self.angle_embedding(batch) #shape: [batch_size of time t+1, dim]?
      # print(f"{t=}: {theta.shape=}, {[history.shape for history in self.initial_history]=}")

      accum = 0
      for lima_num, lima in enumerate(self.limas):
        # print(f'{batch_size=}, {next_batch_size=}, {hippo_c[0].shape=}')
        diff = (theta-accum)[:, None, :]-histories[lima_num][:next_batch_size]
        # print(f'{diff=}, {hippo_c[lima_num][:next_batch_size]=}')
        history = lima(torch.concat([diff, hippo_c[lima_num][:next_batch_size]], dim=-1)) # shape: [batch_size of time t+1, num_heads, dim]
        hippo_c[lima_num] = self.hippo(hippo_c[lima_num][:next_batch_size], history, t+1)
  ...
  def inference(self, input_ids, histories, t, return_updated_history=False, distance='l2'):
    ...
    pdist = self.pairwise_distance(predicts, self.predictor.all_word_embeddings().detach(), distance=distance)

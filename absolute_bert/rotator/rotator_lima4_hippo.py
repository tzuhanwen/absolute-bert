...
class Multihead_Lima(nn.Module):
  def __init__(self, dim=64, hidden_dim=32, hippo_dim=32, num_heads=4, fix_max_value=1):
    super().__init__()
    self.dim = dim
    ...
    self.fc_in = torch.nn.Linear(self.num_heads*(self.dim+self.hippo_dim), self.num_heads*self.hidden_dim, bias=False)
...
class Hippo(nn.Module):
  def __init__(self, dim, c_dim=32):
    super().__init__()
    self.dim = dim
    self.c_dim = c_dim
  ...
  def forward(self, c, h, t):
    f = self.Lf(h.view(-1, ))
    return torch.einsum('bhm,mn', c, self.eye - self.AT/t) + (f * self.B)/t
...
class Rotator(nn.Module):
  def __init__(self, vocab_size, dim=64, hidden_dim=32, hippo_dim=32, num_heads=4, depth=3, rotary_denom=.5, **kwargs):
    super().__init__()
    self.vocab_size = vocab_size
    self.dim = dim
    self.hidden_dim = hidden_dim
    ...
    self.hippo = Hippo(dim=self.dim, c_dim=self.hippo_dim)
    self.initial_c = nn.Parameter(torch.zeros([1, self.num_heads, self.hippo_dim]), requires_grad=False)
  ...
  def forward(self, data, batch_sizes, **kwargs):
    ...
    hippo_c = [self.initial_c.expand(batch_sizes[0], -1, -1) for _ in range(self.depth)]
    ...
    for t, (batch_accum, batch_size, next_batch_size) in enumerate(zip(itertools.accumulate(batch_sizes, initial=0), batch_sizes, batch_sizes[1:])):
      ...
      theta = self.angle_embedding(batch) #shape: [batch_size of time t, dim]?
      ...
      for lima_num, lima in enumerate(self.limas):
        ...
        history = lima(torch.concat([diff, hippo_c[lima_num][:next_batch_size]], dim=-1))
        # yield hippo_c[lima_num][:next_batch_size], history

...
from ..registry import LanguageModelType, lm_summary_generator_registry
...
@lm_summary_generator_registry.register(LanguageModelType.ABSOLUTE_BERT)
class SummaryGenerator:

    def __init__(self, lm: AbsoluteBertLM, tokenizer: PreTrainedTokenizerBase) -> None:
        self.lm = lm
        self.tokenizer = tokenizer

    def __call__(
        self, device: Device = "cpu",
        # sampling_head: int | None = None,
    ) -> dict[LayerName, str]:
        
        layer_semantics = self._extract_absolute_bert_semantics(device)
    ...
    def _extract_absolute_bert_semantics(
        ...
        embeddings = self.lm.word_embeddings

        layer_semantics: list[NamedSemantic] = []
        for layer_num, layer in enumerate(self.lm.base_model.layers):
            head_concepts_tuples = extract_attention_concepts(
                embeddings, layer.attention, self.tokenizer
            )

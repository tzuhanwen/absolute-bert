...
class Multihead_Lima(nn.Module):
  def __init__(self, dim=64, hidden_dim=32, history_dim_in=16, history_dim_out=None, num_heads=4, fix_max_value=1):
    ...
    self.proportion = nn.Parameter(torch.rand(self.num_heads, self.hidden_dim)) 
    self.lima_shape = nn.Parameter(torch.rand(self.num_heads, self.hidden_dim)) # 類似之前的 b/a, 會用 sigmoid 把 range 限制在 -0.25 ~ 0.25
    self.fc_in_w = nn.Parameter(torch.randn(self.num_heads, self.dim+self.history_dim_in, self.hidden_dim))
    self.fc_in_b = nn.Parameter(torch.randn(self.num_heads, self.hidden_dim))
    ...
    self.fc_out_w = nn.Parameter(torch.randn(self.num_heads, self.history_dim_out, self.dim))
    self.fc_out_b = nn.Parameter(torch.randn(self.num_heads, self.dim))
  ...
  def forward(self, theta, histories):
    ...
    total_in = torch.concat([theta[:, None, :].expand(-1, self.num_heads, -1), histories], dim=-1)
    # h = self.fc_in(total_in).view(batch_size, self.num_heads, self.hidden_dim)
    h = torch.einsum('bhm,hmn->bhn', total_in, self.fc_in_w) + self.fc_in_b
    ...
    output = torch.einsum('bhm,hmn->bhn', raw_resulting_angle, self.fc_out_w) + self.fc_out_b
...
class Output(nn.Module):
  def __init__(self, vocab_size, dim=64, rotary_denom=.5, local_coordinating=False, add_time_angles=False, **kwargs):
    ...
    self.embedding_real = nn.Embedding(vocab_size, self.dim, _weight=torch.nn.init.xavier_normal_(torch.ones([vocab_size, self.dim])))
    self.embedding_imag = nn.Embedding(vocab_size, self.dim, _weight=torch.nn.init.xavier_normal_(torch.ones([vocab_size, self.dim])))
...
class Rotator(nn.Module):
  ...
  def forward(self, data, time, histories, **kwargs):
    ...
    return scale[:, None] * predicts, { 'histories': new_histories }

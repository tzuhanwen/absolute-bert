...
class Lima(nn.Module):
  def __init__(self, dim=64, fix_max_value=1):
    super().__init__()
    self.dim = dim
    ...
    self.proportion = torch.nn.Parameter(torch.rand(1, self.dim)) # weight 的 range 會限制在 M 下面的一個比例: 1/(e^{proportion}+1)
    self.lima_shape = torch.nn.Parameter(torch.rand(1, self.dim)) # 類似之前的 b/a, 會用 sigmoid 把 range 限制在 -0.25 ~ 0.25
  ...
  def forward(self, theta):
    b_a = (torch.sigmoid(self.lima_shape) - 0.5) / 2  # b/a
    o = self.M + (self.M/(2*(torch.pow(torch.e, self.proportion)+1))) * (-1 - b_a + theta.cos() + b_a*(2*theta).cos())
...
class Output(nn.Module):
  ...
  def forward(self, histories, sources, t):
    metric_theta = 1/self.rotary_denom**(self.dimension_nums/self.dim)
    total_angles = t*metric_theta + histories
...
class Rotator(nn.Module):
  ...
  def forward(self, data, batch_sizes, **kwargs):
    ...
    histories = [self.initial_history] * len(self.limas)
    ...
    for t, (batch_accum, batch_size, next_batch_size) in enumerate(zip(itertools.accumulate(batch_sizes, initial=0), batch_sizes, batch_sizes[1:])):
      ...
      for lima_num, lima in enumerate(self.limas):
        diff = theta-histories[lima_num][:theta.shape[0], :]
      ...
      predicts = self.predictor(accum, batch, t)

...
class Multihead_Lima(nn.Module):
  def __init__(self, dim=64, head_num=4, fix_max_value=1):
    ...
    self.Theta = torch.nn.Parameter(torch.rand(self.head_num, self.dim, self.dim)*torch.pi*2 - torch.pi, requires_grad=True)
...
class Rotator(nn.Module):
  def __init__(self, vocab_size, dim=64, head_num=4, depth=3, rotary_denom=.5, **kwargs):
    super().__init__()
    self.vocab_size = vocab_size
    self.dim = dim
    self.head_num = head_num
    ...
    self.initial_history = nn.ParameterList(
        [torch.nn.Parameter(torch.randn(self.head_num, 1, self.dim), requires_grad=True) for _ in range(self.depth)]
    )
    ...
    self.head_clock_denoms = torch.nn.Parameter(torch.tensor([.5]*self.head_num))
    self.layer_norm = nn.LayerNorm(self.dim)
  ...
  def forward(self, data, batch_sizes, **kwargs):
    ...
    for t, (batch_accum, batch_size, next_batch_size) in enumerate(zip(itertools.accumulate(batch_sizes, initial=0), batch_sizes, batch_sizes[1:])):
      ...
      for lima_num, lima in enumerate(self.limas):
        ...
        accum += self.layer_norm(history.sum(dim=0))
